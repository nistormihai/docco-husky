(function() {
  ...
  parse_args = function(args,  callback) {
    var a, args, ext, lang_filter, project_name, output_dir, roots;
    ...
    return exec("find " + roots + " -type f \\( " + lang_filter + " \\)", function(err, stdout) {
      ...
      return callback(sources, project_name, output_dir, args);
    });
  };
  ...
  check_config = function(context, pkg) {
    ...
    defaults = {
      ...
      show_timestamp: true,
      output_dir: context.options.output_dir || "docs",
  ...
  var document = function(options, callback){
    parse_args(  options,function(sources, project_name, output_dir, raw_paths) {
